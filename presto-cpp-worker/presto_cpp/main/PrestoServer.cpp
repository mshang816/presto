...
void PrestoServer::initializeAsyncCache() {
  auto nodeConfig = NodeConfig::instance();
  auto memoryGbString =
      util::getOptionalProperty(nodeConfig->values(), "node.memory_gb", []() {
        return fmt::format("{}", FLAGS_system_memory_gb);
      });
  uint64_t memoryGb = atoi(memoryGbString.c_str());
  if (!memoryGb) {
    LOG(ERROR) << "Bad memory size " << memoryGbString;
    exit(1);
  }
...
void PrestoServer::stop() {
  ...
  if (not shuttingDown_.exchange(true)) {
    ...
    // any tasks.
    usleep(FLAGS_shutdown_onset_sec * 1'000'000); // 10 seconds, by default.
