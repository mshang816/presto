...
void getData(
  ...
  bufferManager.getData(
      ...
      [taskId = taskId, bufferId = bufferId, promiseHolder](
        ...
        velox::serializer::presto::PrestoOutputStreamListener listener;
        ...
        VLOG(1) << "Task " << taskId << ", buffer " << bufferId << ", sequence "
                << sequence << " Results size: " << data.size()
                << ", page count: " << groups.size()
        ...
        result->data = folly::IOBuf::copyBuffer(data);
...
std::unique_ptr<TaskInfo> TaskManager::deleteTask(
  ...
  if (execTask) {
    ...
    if (state == exec::kRunning) {
      execTask->terminate(exec::kAborted);
    }
