    ...
    if (buffer->isChained()) {
      ...
      while (nextBuffer) {
        byteRanges.emplace_back(ByteRange{
            const_cast<uint8_t*>(nextBuffer->buffer()),
            (int32_t)nextBuffer->length(),
            0});
        nextBuffer = nextBuffer->next();
      }
    } else {
      byteRanges.emplace_back(ByteRange{
          const_cast<uint8_t*>(buffer->buffer()),
...
class TaskManagerTest : public testing::Test {
 protected:
  ...
  std::unordered_map<std::string, std::string> configs_{
      {TaskManager::kMaxDriversPerTask.toString(), "1"}};
...
TEST_F(TaskManagerTest, outOfQueryUserMemory) {
  ...
  taskManager_->getQueryContextManager()->overrideProperties(
      "query.max-memory-per-node", fmt::format("{}B", peakUser - 1));
  taskManager_->getQueryContextManager()->overrideProperties(
  ...
  taskManager_->getQueryContextManager()->overrideProperties(
      "query.max-total-memory-per-node", fmt::format("{}B", peakTotal - 1));
